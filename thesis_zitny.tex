...
%% GLOSSARIES
%%
%% TODO framework, api, cloud, os, linux, unix, unix-like, ict, php, server-side, languages
%% iaas, paas, saas, framework
\makeglossaries
...
{
  name=unix-like,
  description={referrence to operating systems that are similar to Unix}
}
\newglossaryentry{GUI}
{
  name=GUI,
...
{
  name=Ubuntu,
  description={distribution of Linux operating system}
}
...
Basics of these thoughts are going to be the essential part of the following work. The idea was initiated when comparing several Linux distributions at distrowatch.com -- the current number one \uv{comparator} operating systems. Read along to unveil the challenges this idea had met and defeated.
\end{introduction}
...
Let us call the would-be website \uv{The New Distrowatch} (TND) and start with specific functions it should provide.
...
%TODO However, to get attention from these people, the final website should present the information interestingly. The website's user interface should be intuitive and easy-to-use. The quantity of information must be shown in simple way. This would be another asset in comparing with the original distrowatch.com. Graphic identity is covered in LINK.

\section{Comparsion to distrowatch.com}

%TODO The great idea is… the current status is … the problem is… the missing pieces to be a perfect are… but great foundation and userbase… \\
...
Since this work will not be dependent only on the web layer, the preferred language should be capable of doing \uv{the backend work}. It could be a scripting language, however, it should provide great number of libraries built on top. Since \uv{the backend work} would sometimes need to be done quickly by tiny executables, the list will have to be reduced by Java and C\#.
...
So \uv{the chosen one} is Python in the newest stable release -- 3.3, however, the frontend will be implemented in PHP due to security and easiness issues which will be detailed in the following chapter.
...
%\begin{figure}[h!]
%	\includegraphics[trim=0.2cm 0cm 0cm 0cm, clip=true, totalheight=280pt]{images/screen-home-1.png}
%	\caption{Screenshot of frontend web}
...
\begin{itemize}
	...
	\item \textbf{Distros} list all operating systems that are stored in database. After clicking on one of them, the \uv{profile page} of that system is shown.
...
%Framework Django mal základnú výhodu - jazyk Python, ktorý bol vyššie zvolený za preferovaný. Hneď po tom však prišla nevýhoda, ktorou je neúplná podpora verzie Python 3, ktorá sa bude používať a značne sa odlišuje od svojho predchodcu. Medzi ďalšie výhody patrí bohatá vývojárska základňa, komunita, dokumentácia, množstvo implementovaných riešení vo forme modulov (v Djangu volaných "apps"). Oproti populárnym frameworkom v PHP však kladie menší dôraz na bezpečnosť. Python 3 - wtf, história a workaround..
...
%TODO Symfony patrí medzi najpoužívanejšie a najbezpečnejšie riešenia používané na tvorbu väčšich webových projektov. Veľká časť jeho "vnútorností" sa používa aj samostatne, pretože je známkou ……. A má veľa už naimplementovaných jednoducho zapojiteľných modulov tretích strán. \\
...
%TODO vymyslieť spôsob automatizovanej inštalácie jenotne pre všetky sys.
...
Obtaining \uv{backup} information from trusted sources on the internet is done thanks to Python libraries lxml and requests. Scanning the sources and parsing data is done by multiprocessing units and saved into database via Django ORM Models interface.
